...
export default function EducationPage() {
  ...
  return (
    <main className="max-w-5xl w-full mx-auto px-4 sm:px-6 pt-6 sm:pt-10 pb-20 space-y-12 overflow-hidden">
      ...
      {/* Quick Overview Horizontal Banner */}
      ...
      <div className="space-y-6">
        {educationList.map((edu) => (
          <div
            key={edu.id}
            className="p-6 sm:p-8 rounded-2xl border border-zinc-800/80 bg-zinc-950/70 hover:bg-zinc-900/40 hover:border-zinc-700/80 transition-all duration-300 shadow-sm space-y-4"
          >
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
              <div className="flex items-start gap-3">
                <div
                  className={`w-10 h-10 rounded-xl border font-mono text-xs font-bold flex items-center justify-center shrink-0 ${edu.badgeBg}`}
                >
                  {edu.initials}
                </div>
              ...
              <div className="flex items-center gap-1.5 text-xs font-mono text-zinc-300 bg-zinc-900 px-3 py-1 rounded-lg border border-zinc-800 shrink-0 self-start sm:self-auto">
